...
var arraySKillsUniqSort = _.sortBy(_.uniq(arraySkills), function(sort){
    return sort.charCodeAt();
});
console.log('arraySKillsUniqSort', arraySKillsUniqSort);

//2
var nameSort = _.sortBy(data, ['friends.length', 'name']);
var arrayNames = _.map(nameSort, 'name');
console.log('arrayNamesFriendsSort', arrayNames);
...
_.each(data, function(item){
    _.each(item.friends, function(friend){
        arrayFriends.push(friend.name.toLowerCase());
    });
});
var arrayFriendsUniqSort = _.sortBy(_.uniq(arrayFriends), function(sort){
    return sort.charCodeAt();
});
console.log('arrayFriendsUniqSort', arrayFriendsUniqSort);
